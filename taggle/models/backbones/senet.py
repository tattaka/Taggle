...
class SEBottleneck(Bottleneck):
    ...
    def __init__(self, inplanes, planes, groups, reduction, stride=1,
                 downsample=None, dropblock_fn=None):
...
class SEResNeXtBottleneck(Bottleneck):
    ...
    def __init__(self, inplanes, planes, groups, reduction, stride=1,
                 downsample=None, base_width=4, dropblock_fn=None):
...
class SENet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, groups, reduction, stride=1,
                    downsample_kernel_size=1, downsample_padding=0, dropblock_fn=None):
        downsample = None
        if dropblock_fn is None:
            dropblock_fn = nn.Identity()
        ...
        layers.append(block(self.inplanes, planes, groups, reduction, stride,
                            downsample, dropblock_fn=dropblock_fn))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups,
                                reduction, dropblock_fn=dropblock_fn))
...
#     def forward(self, x, ex_index=None):
    def forward(self, x):
...
#             x1 = moex(x1, ex_index=ex_index, moex_norm='pono', moex_epsilon=1e-5, moex_positive_only=False) # WIP
        x2 = self.layer2(x1)
        x3 = self.layer3(x2)
        x4 = self.layer4(x3)
