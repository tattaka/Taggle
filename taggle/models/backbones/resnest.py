...
from .resnest_util import SplAtConv2d, resnest_model_urls
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
        ...
        if dropblock_prob > 0.0:
            self.dropblock1 = LinearScheduler(
            ...
            if radix == 1:
                self.dropblock2 = LinearScheduler(
                    DropBlock2D(block_size=7, drop_prob=0.),
                    start_value=0.,
                    stop_value=dropblock_prob,
                    nr_steps=5
                )
            self.dropblock3 = LinearScheduler(
                DropBlock2D(block_size=7, drop_prob=0.),
                start_value=0.,
                stop_value=dropblock_prob,
                nr_steps=5
...
def _get_pretrained_settings(backbone):
    urls = resnest_model_urls
    pretrained_settings = {
        'imagenet': {
...
resnest_backbones = {
    'resnest50': {
        ...
        'pretrained_settings': _get_pretrained_settings('resnest50'),
    ...
    'resnest101': {
        'backbone': ResNeStBackbone,
        'pretrained_settings': _get_pretrained_settings('resnest101'),
        'out_shapes': (2048, 1024, 512, 256, 64),
        'params': {
            'block': Bottleneck,
            'layers': [3, 4, 23, 3],
    ...
    'resnest200': {
        'backbone': ResNeStBackbone,
        'pretrained_settings': _get_pretrained_settings('resnest200'),
        'out_shapes': (2048, 1024, 512, 256, 64),
        'params': {
            'block': Bottleneck,
            'layers': [3, 24, 36, 3],
    ...
    'resnest269': {
        'backbone': ResNeStBackbone,
        'pretrained_settings': _get_pretrained_settings('resnest269'),
        'out_shapes': (2048, 1024, 512, 256, 64),
        'params': {
            'block': Bottleneck,
            'layers': [3, 30, 48, 8],
