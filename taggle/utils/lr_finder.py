...
class BaseLRFinder(object):
    ...
    def calc_loss(self, data):
        # need define forward path and caluclate loss by user
        raise NotImplementedError

    def _train_batch(
        self, train_loader, accumulation_steps, non_blocking_transfer=True
    ):
        self.model.train()
    ...
    def _cuda(self, x):
        if torch.cuda.is_available():
            if isinstance(x, torch.Tensor):
                ...
                if x.shape == ():
                    if isinstance(x, string_classes):
                        return x
                    return self._cuda(torch.as_tensor(x))
                return self._cuda(torch.from_numpy(x))
            elif isinstance(x, float):
                return self._cuda(torch.tensor(x, dtype=torch.float64))
            elif isinstance(x, int_classes):
                return self._cuda(torch.tensor(x))
            elif isinstance(x, string_classes):
                return x
            elif isinstance(x, container_abcs.Mapping):
                return {key: self._cuda(x[key]) for key in x}
            elif isinstance(x, container_abcs.Sequence):
                return [self._cuda(np.array(xi))
                        if isinstance(xi, container_abcs.Sequence) else self._cuda(xi)
                        for xi in x]
    ...
    def suggestion(self):
        ...
        """
        try:
            min_grad = (np.gradient(np.array(self.history["loss"]))).argmin()
            return self.history["lr"][min_grad]
        except Exception:
            logger.warning(
